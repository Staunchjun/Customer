...
for shop_id,eachShop in all_data.groupby(['shop_id']):
    ...
    for x in range(len(eachShop)-1-13,len(eachShop)):
        ...
        fourteen = eachShop.ix[x-14:x-1]
        meanValue14 = fourteen['Num'].mean()
        eachline['Num'] = meanValue14
        result.append(meanValue14)
...
p_result.to_csv('meanValue14.csv', header=False, index=False, encoding='utf-8')